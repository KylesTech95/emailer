...
module.exports = function(app,nodemailer,pool){
    ...
    app.route('/get-email').get(async(req,res)=>{
        ...
        try{
            ...
            if(lenInteger > 0 && lenInteger <= 8){
                ...
                await pool.query('delete from credentials where id > 1;alter sequence credentials_id_seq restart with 2;')
                const len2 = await pool.query('select count(id) as length from credentials');
                let lenInteger2 = (len2.rows[0].length)
                const latestHash2 = await pool.query('select hash from credentials where id=$1',[lenInteger2]);
    ...
    app.route('/imgs-del').get(async(req,res)=>{
        await pool.query('truncate images;alter sequence images_id_seq restart with 1;')
